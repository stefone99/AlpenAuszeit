...
struct CurrentView: View {
    ...
    var body: some View {
        ScrollView {
            ...
            ZStack(alignment: .top) {
                ...
                SemicircleShape(radiusFactor: circleRadiusFactor)
                    ...
                    .edgesIgnoringSafeArea(.top) // Wichtig: Ignoriert den oberen sicheren Bereich
        ...
        .edgesIgnoringSafeArea(.top) // Wichtig: Lässt den Halbkreis bis zum oberen Bildschirmrand reichen
        .onAppear {
...
// Verbesserte Halbkreis-Form mit einstellbarem Radiusfaktor
struct SemicircleShape: Shape {
    var radiusFactor: CGFloat = 0.6
    
    func path(in rect: CGRect) -> Path {
        var path = Path()
        path.addArc(
            center: CGPoint(x: rect.midX, y: 0),
            radius: rect.width * radiusFactor, // Einstellbarer Radius
            startAngle: .zero,
